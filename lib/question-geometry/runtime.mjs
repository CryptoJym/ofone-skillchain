...
function completedPasses(state, causalResult) {
  ...
  for (const event of array(state.history)) {
    if (!answerContextQualifies(event.answer_context)) continue;
    const tags = array(event.pass_tags).length > 0
      ? array(event.pass_tags)
      : array(questionIndex.get(event.question_id)?.pass_tags);
    for (const tag of tags) passes.add(tag);
  }
...
export function evaluateConvergence(state) {
  ...
  for (const finding of verifyHistoryChain(state)) {
    blockers.push({ code: finding.code, detail: `history[${finding.index}]` });
  }
  ...
  const waiverRequested = state.convergence?.robustness_waiver === true;
  const waiverOwner = String(state.convergence?.waived_by || "").trim();
  const waiverActive = waiverRequested && waiverOwner.length > 0;
  if (waiverRequested && !waiverActive) {
    blockers.push({ code: "QG_WAIVER_NOT_OWNED", detail: "robustness_waiver requires a named waived_by owner" });
  }
  if (metrics.decision_robustness < policy.thresholds.minimum_decision_robustness && !waiverActive) {
    blockers.push({ code: "QG_DECISION_NOT_ROBUST", detail: metrics.decision_robustness });
  }
  if (metrics.residual_evpi > policy.thresholds.maximum_residual_evpi && !waiverActive) {
...
export function applyAnswer(inputState, questionId, answer, externalEffects = null, answerContext = null) {
  const state = clone(inputState);
  state.policy = policyFor(state);
  state.questions = array(state.questions);
  state.history = array(state.history);
  ...
  if (question.status !== "selected" || state.policy.current_question_id !== questionId) {
    throw new Error(`Question ${questionId} is not the runtime-issued selection; run step first and answer only the issued question`);
  }
  ...
  question.answer_context = answerContext ? clone(answerContext) : null;
  ...
  state.history.push(chainEvent(state, {
    ...
    answer,
    answer_context: answerContext ? clone(answerContext) : null,

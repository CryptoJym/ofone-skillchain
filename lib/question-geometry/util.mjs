...
export const PROVENANCE_SOURCE_TYPES = [
  "human", "human_review", "tool", "file", "url", "observation",
  "simulation", "benchmark_oracle", "test_fixture", "agent_inference"
];

export const RELIABILITY_LEVELS = ["low", "medium", "high"];
export const PASS_OUTCOMES = ["satisfied", "not_satisfied", "inconclusive"];
export const PASS_BASIS_TYPES = [
  "evidence", "counterfactual", "intervention", "formal_proof",
  "independent_review", "human_judgment"
];
...
export function reliabilityRank(value) {
  return RELIABILITY_LEVELS.indexOf(value);
}

export function reliabilityAtLeast(actual, minimum) {
  return reliabilityRank(actual) >= reliabilityRank(minimum);
}

export function isSubstantive(value, minimumLength = 8) {
  const text = String(value || "").trim();
  if (text.length < minimumLength) return false;
  return !/^(?:none|n\/?a|na|unknown|tbd|todo|same|not applicable|no evidence)$/i.test(text);
}

import { QUESTION_TYPES, answerContextQualifies, array, object, policyFor } from "./util.mjs";
import { evaluateCausalDepth } from "./causal.mjs";
import { verifyHistoryChain } from "./state.mjs";
...
export function validateQuestionGeometry(state) {
  ...
  for (const chainFinding of verifyHistoryChain(state)) {
    add("error", chainFinding.code, chainFinding.message, `history[${chainFinding.index}]`);
  }
  for (const question of array(state.questions)) {
    if (question.status === "answered"
      && array(question.pass_tags).length > 0
      && !answerContextQualifies(question.answer_context)) {
      add("warning", "QG_PASS_CONTEXT_MISSING",
        "Answered challenge-pass question has no qualifying answer context; the pass will not count toward convergence.",

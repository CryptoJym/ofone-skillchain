...
For recursive external review, benchmark frontier runs, or ChatGPT Deep Research acquisition, use the Chrome extension/plugin as the launch and observation surface. Prefer extension-managed clean isolated tabs driven by `research/deep-research-extension-payloads.json`, and require extension observations to land in `research/deep-research-extension-report.json`, so multiple Deep Research packets can run without taking over the user's active workspace. In Codex Desktop, the expected Chrome plugin callable surface may be the `node_repl` bridge with `globalThis.browser`, not a standalone `mcp__chrome__*` namespace: expose `node_repl` through tool discovery, then probe `nodeRepl.requestMeta["x-codex-browser-use-available-backends"]`, `browser.browserId`, `browser.capabilities`, and `browser.tabs.list()`. A missing standalone Chrome namespace is a troubleshooting datum, not a fallback authorization, if `node_repl` can see the `chrome` backend and list Chrome tabs. Do not use Browser, Computer Use, coordinate clicking, AppleScript/JXA, or generic desktop automation to take over the user's active browser for launch or harvest.

When troubleshooting the current Chrome bridge, preserve the observed method shapes. `browser.tabs.list()` returns minimal tab summaries; use `browser.tabs.get(tabId)` for the richer tab object with `playwright`, `dom_cua`, `content`, `clipboard`, and `dev` helpers. `browser.tabs.content({ urls, contentType })` uses camelCase `contentType`, but the Chrome backend can still reject `tabs_content`; `tab.content.export()` takes no format argument and may be unsupported; `tab.content.exportGsuite(format)` takes a format string and only applies to Google Workspace documents. Treat signature or backend errors as harvest-probe evidence, not as permission to switch to a desktop-control fallback.

If the Chrome extension bridge is callable but a Deep Research submission settles into a no-start state, classify that as a Deep Research handoff failure, not Chrome-extension unavailability. No-start evidence includes only the submitted user prompt being visible after submission, an internal Deep Research iframe with an empty body, repeated `Ignoring message from unknown source MessageEvent` logs, and no generated plan, Start/countdown action, active research state, or stop-control evidence. A minimal smoke-test prompt may be used only to isolate whether the handoff failure is prompt-specific; it is never benchmark evidence, launch proof, harvest proof, or permission to retry same-shape benchmark launches without a changed launch condition.

If extension control is unavailable or clean tab isolation cannot be verified, stop normal workflow and troubleshoot extension availability first, before doing any benchmark, harvest, launch, or repo-promotion work: record tool discovery, `nodeRepl.requestMeta` backend evidence, `globalThis.browser` presence or absence, `browser.tabs.list()`/tab-list attempt or error, and the exact blocked namespace. Do not treat a missing `browser.tabs.query()` helper as extension failure when the current Chrome bridge exposes `browser.tabs.list()`. If a completed report is visible but raw Markdown is blocked, record each allowed Chrome-extension harvest probe before leaving the item `completed_report_visible`; when the extension can observe completion but cannot read/export the cross-origin report body, bind `research/deep-research-manual-recovery.json` to the current queue/payload/report hashes and use it to gate any native ChatGPT Markdown export before local review. Only then may the queue/payload/report item remain `prepared`, `blocked`, or `completed_report_visible`; do not mark it launched, harvested, reviewed, complete, or aggregate-eligible without the matching machine-checkable proof.
...
```text
prepared -> launched -> active_researching -> completed_report_visible -> harvested -> accepted|rejected -> integrated|deferred -> resubmitted|converged|blocked
```

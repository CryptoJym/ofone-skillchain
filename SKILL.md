...
For recursive external review, benchmark frontier runs, or ChatGPT Deep Research acquisition, use the Chrome extension/plugin as the launch and observation surface. Prefer extension-managed clean isolated tabs driven by `research/deep-research-extension-payloads.json`, and require extension observations to land in `research/deep-research-extension-report.json`, so multiple Deep Research packets can run without taking over the user's active workspace. Do not use Browser, Computer Use, coordinate clicking, AppleScript/JXA, or generic desktop automation to take over the user's active browser for launch or harvest unless the user explicitly authorizes a one-off manual assist. If extension control is unavailable or clean tab isolation cannot be verified, stop normal workflow and troubleshoot extension availability first: record tool discovery, `nodeRepl.requestMeta` backend evidence, `globalThis.browser` presence or absence, tab-list attempt or error, and the exact blocked namespace before doing anything else. If a completed report is visible but raw Markdown is blocked, record each allowed Chrome-extension harvest probe before leaving the item `completed_report_visible`; when the extension can observe completion but cannot read/export the cross-origin report body, bind `research/deep-research-manual-recovery.json` to the current queue/payload/report hashes and use it to gate any native ChatGPT Markdown export before local review. Only then may the queue/payload/report item remain `prepared`, `blocked`, or `completed_report_visible`; do not mark it launched, harvested, reviewed, complete, or aggregate-eligible without the matching machine-checkable proof.

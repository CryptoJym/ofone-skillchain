...
For recursive external review, benchmark frontier runs, or ChatGPT Deep Research acquisition, use the Chrome extension/plugin as the launch and observation surface. Prefer extension-managed clean isolated tabs driven by `research/deep-research-extension-payloads.json` so multiple Deep Research packets can run without taking over the user's active workspace. Do not use Browser, Computer Use, coordinate clicking, AppleScript/JXA, or generic desktop automation to take over the user's active browser for launch or harvest unless the user explicitly authorizes a one-off manual assist. If extension control is unavailable or clean tab isolation cannot be verified, leave the queue/payload item `prepared` or `blocked`; do not mark it launched, harvested, reviewed, complete, or aggregate-eligible.

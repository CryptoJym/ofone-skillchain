...
- Next bounded action: launch or observe the prepared `case-formal-proof-search-001` / `frontier_reasoning` / repeat 1 direct-answer arm only after callable Chrome extension/plugin control can verify clean ChatGPT Deep Research launch proof. Do not use Browser, Computer Use, coordinate clicking, AppleScript/JXA, or generic desktop automation as fallback. For the repaired strategic and regulated wastewater frontier full-OfOne slots, do not launch another same-shape Deep Research remedial rerun. Superiority claims remain blocked.

...
- launch surface: Chrome extension/plugin control with clean isolated tab/conversation proof. If unavailable, the run remains `prepared` or `blocked`; do not use Browser, Computer Use, coordinate clicking, AppleScript/JXA, or generic desktop automation as a fallback launch path. Prepared launchable work should be represented in `research/deep-research-launch-queue.json` and validated with `npm run deep-research:check`.

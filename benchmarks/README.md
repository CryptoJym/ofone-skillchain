...
Its status is `in_progress`; forty-five local `agentic_coding` raw outputs have unblinded local reviews across the strategic gated diligence repeat-1, repeat-2, and repeat-3 slices, scientific mechanism repeat-1, repeat-2, and repeat-3 slices, regulated wastewater repeat-1, repeat-2, and repeat-3 slices, formal proof-search repeat-1, repeat-2, and repeat-3 slices, and public-sector AI policy audit repeat-1, repeat-2, and repeat-3 slices.
...
The regulated wastewater repeat-3 slice now has direct, light-structured, and full-OfOne outputs; the full-OfOne artifact is case-native and includes validator, Map rendering, patch report, and local review files.
...
The first `frontier_reasoning` strategic gated diligence repeat-1 slice has direct-answer, light-structured, and full-OfOne outputs. The direct-answer and light-structured frontier outputs are harvested, reviewed, and aggregate-eligible. The full-OfOne frontier output is harvested, reviewed, and excluded before aggregate scoring because computed local semantic validation failed. Four remedial frontier full-OfOne attempts are preserved outside aggregate scoring: rerun 1 and rerun 4 returned meta/advisory reports rather than benchmark packages, rerun 2 failed computed evidence/tradeoff validation, and rerun 3 failed current-schema `benchmark_trace` plus relation-legality validation. Same-shape attachment-led Deep Research reruns are now barred for this slot. The Mode A controlled execution contract at `benchmarks/runs/2026-05-17-batch-01/frontier-run-packets/2026-05-21-strategic-gated-diligence-frontier-full-r1-mode-a-contract.md` produced rerun 5; its raw output, artifact JSON, computed validator JSON, rendering, patch report, and local review are recorded in `remedial_runs` as `replace_for_aggregate_only`. It does not create a new predeclared slot and does not support superiority claims by itself.
The `frontier_reasoning` regulated wastewater repeat-1 slice has direct-answer, light-structured, and full-OfOne outputs. The direct-answer and light-structured outputs are harvested, reviewed, and aggregate-eligible. The original full-OfOne output is harvested, reviewed, and excluded before aggregate scoring because computed local semantic validation failed relation legality and relation-family checks. The Mode A controlled execution contract at `benchmarks/runs/2026-05-17-batch-01/frontier-run-packets/2026-05-21-regulated-wastewater-frontier-full-r1-mode-a-contract.md` produced controlled rerun 1; its raw output, artifact JSON, computed validator JSON, rendering, patch report, and local review are recorded in `remedial_runs` as `replace_for_aggregate_only`. It does not create a new predeclared slot and does not support superiority claims by itself.
The `frontier_reasoning` formal proof-search repeat-1 slice is active at `benchmarks/runs/2026-05-17-batch-01/frontier-run-packets/2026-05-21-formal-proof-search-frontier-r1.md`. The direct-answer arm completed in ChatGPT Deep Research at https://chatgpt.com/c/6a0f0a85-c75c-83e8-b0d0-4c15a041cb7b through the Chrome extension plugin and is harvested, reviewed, and aggregate-eligible. The light-structured arm completed visibly at https://chatgpt.com/c/6a0f1fe5-3494-83e8-9f92-1a2b732c4958 with `Research completed in 9m`, `6 citations`, `120 searches`, and report title `Benchmark Raw Output`, but raw Markdown harvest is blocked by the cross-origin Deep Research iframe. The light-structured arm is `completed_report_visible`, not reviewed, complete, aggregate-eligible, or publishable.
